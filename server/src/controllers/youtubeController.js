...
const related_to_video = async (req, res) => {
    try {
...
const video_full_data = async (req, res) => {
    try {
        console.log('in video_full_data')
        let video_id = req.body.video_id;
        console.log(video_id)
            let video_data =  await service.videos.list({
                "part" : ["snippet , contentDetails , statistics"],
                "id" : video_id
            });
                let obj = {
                    videoId: video_data.data.items[0].id,
                    url : video_data.data.items[0].snippet.thumbnails.standard.url,
                    title : video_data.data.items[0].snippet.title,
                    publishedAt:video_data.data.items[0].snippet.publishedAt,
                    duration:ytDurationFormat(video_data.data.items[0].contentDetails.duration),
                    description : video_data.data.items[0].snippet.description,
                    commentCount :video_data.data.items[0].statistics.commentCount, 
                    tags: video_data.data.items[0].snippet.tags,
                    channelTitle : video_data.data.items[0].snippet.channelTitle,
                    commentCount: video_data.data.items[0].statistics.commentCount,
                    favoriteCount: video_data.data.items[0].statistics.favoriteCount,
                    likeCount: video_data.data.items[0].statistics.likeCount,
                    viewCount: video_data.data.items[0].statistics.viewCount
                };
...
const video_full_data_demo = async (req, res) => {
    try {
        ...
        console.log(video_id)
            ...
            res.send({success: true, data:video_data});
    } catch (e) {
        console.log('in v catch')
        console.log(e.message)
        res.send({success: false, error: e.message});
...
const demo_comments = async (req, res) => {
    setTimeout(function () {
const json = [{
...
const ans_json = json.map(function(el) {
  obj = {
   imgSrc : el.snippet.topLevelComment.snippet.authorProfileImageUrl,
   text : el.snippet.topLevelComment.snippet.textDisplay,
   name : el.snippet.topLevelComment.snippet.authorDisplayName,
...
//    ans = JSON.parse(json);
   res.send({success: true, comments:ans_json});
}, 1000);
};
